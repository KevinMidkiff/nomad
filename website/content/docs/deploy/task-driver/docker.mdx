...
- `default_cap_add` - A list of Linux capabilities to add to every Docker task
  on the client. Each capability must be allowed by `allow_caps`. Task-level
  `cap_add` entries are applied in addition to this list.
